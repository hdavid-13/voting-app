...
export default async function Dashboard() {
  ...
  const open = proposals?.filter((p) => p.status === 'OPEN').length ?? 0
  const totalVotes = proposals?.reduce(
    (acc, p) => acc + p.yes_count + p.no_count + p.abstain_count,
    0
  ) ?? 0
  ...
  return (
    <div className="container">
      ...
      <div style={{ marginBottom: '24px' }}>
        <h1 style={{ fontSize: '1.5rem', color: 'var(--gold)' }}>
          Résultats des votes
        </h1>
      </div>
            ...
            <div key={p.id} className="card">
              ...
              <div style={{ marginTop: '1rem' }}>
                <div style={{ display: 'flex', height: '6px', borderRadius: '2px', overflow: 'hidden', background: '#1e2a38' }}>
                  {yesPct > 0 && (
                    <div style={{ width: `${yesPct}%`, background: 'var(--accent)' }} />
                  )}
                  {abstainPct > 0 && (
                    <div style={{ width: `${abstainPct}%`, background: 'var(--gold)' }} />
                  )}
                  {noPct > 0 && (
